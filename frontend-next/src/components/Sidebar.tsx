...
export default function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
    ...
    const allCategories = SECTIONS.flatMap((s: any) => s.categories);

    const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
        const initial: Record<string, boolean> = {};
        allCategories.forEach((cat: any) => {
            if (cat.tools.some((t: any) => location.pathname === t.path)) {
    ...
    React.useEffect(() => {
        setExpanded((prev: any) => {
            ...
            let changed = false;
            allCategories.forEach((cat: any) => {
                if (cat.tools.some((t: any) => pathname === t.path)) {
                    if (!next[cat.name]) {
                        next[cat.name] = true;
                        changed = true;
                    }
    ...
    const toggleCategory = (name: string) => {
        if (collapsed) {
            onToggle();
            setExpanded((prev: any) => ({ ...prev, [name]: true }));
            return;
        }
        setExpanded((prev: any) => ({ ...prev, [name]: !prev[name] }));
    };
                    ...
                    {SECTIONS.map((section: any) => (
                        <div key={section.label}>
                            ...
                            {section.categories.map((cat: any) => {
                                ...
                                const isActiveCategory = hasTools && cat.tools.some((t: any) => pathname === t.path);
